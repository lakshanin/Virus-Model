...
to setup
  clear-all
  reset-ticks
  create-turtles Population                        ; create turtles
    [ setxy random-xcor random-ycor
      set shape "person"                           ; setting shape person
      set color green                              ; setting color green
      set infected-time 0                          ; setting infected time 0
      set infected? false                          ; is not infected
  ]
  ask n-of (3 * Population / 100) turtles[         ; initialize the infected people to 3% of the population
      set infected? true                           ; is infedted
      set color red                                ; changing color to red
  ]
  if count turtles > 0[
    set %infected (count turtles with [ infected? ] / count turtles) * 100                   ;updating infected people
  ]
end

to go
  ask turtles [
    rt random 100           ; move turtles to random places
    lt random 100
    fd 1
  ...
  tick
  if %infected = 0 [                       ; stopping if infected rate is 0
  set healthy Population - diedNumber      ;setting healthy number
  stop
  ]
  if %infected = 100[stop]                 ; stopping if infected rate is 100
end

to increase-infected-time                   ;setting infected time
  if infected? [
    set infected-time infected-time + 1
  ]
end

to infect                                    ; infecting people
  ask other turtles-here with [ not infected? ][
    if random-float 100 < Infect_rate [
      set infected? true
      set color red
    ]
  ]
...
to kill                                       ; to kill people
  if infected-time > Recovery_duration[
    set diedNumber diedNumber + 1
    die
  ]
end
@#$#@#$#@
...
"" ""
PENS
"Total" 1.0 0 -14737633 true "" "plot count turtles"
"Healthy" 1.0 0 -10899396 true "" "plot count turtles with [ not infected? ]"
